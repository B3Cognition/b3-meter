/**
 * T029 — Undo/Redo E2E Tests
 *
 * Exercises the temporal (zundo) undo/redo stack on testPlanStore at scale:
 *   - 50 add-node operations → undo all 50 → tree is empty
 *   - Redo all 50 → tree has 50 nodes
 *   - Each individual undo/redo operation is measured to be fast (< 50 ms)
 *
 * The 50-operation limit matches the UNDO_LIMIT constant in testPlanStore.ts.
 */
